...
public class Strategy {
    ...
    */
    ...
    private float alphabetaweak(Board board, int depth, int turn, Player player, float alpha, float beta) {
        if (board.isWin() || board.isDraw() || board.noEmpty() || board.passP(turn) || depth == 0) {
            float score = board.evaluate(player, turn);
            return score;
        }
        ...
        for (int i = 0; i < NxN; i++) {
            ...
            if (board.canPut(s)) {
                flipped = board.setBoard(s);
                float score = alphabetaweak(board, depth - 1, -turn, player, alpha, beta);
                board.setAgain(flipped, -turn);
                if (turn == MAX) {
                    beta = Float.min(score, beta);
                    if( beta <= alpha )break;
        ...
        if(turn==MAX)return beta;
        return alpha;
        //if(turn==MAX)return alpha;
        //return beta;
    }
    
    int bestMoveABweak(Board board, Player player) {
        float bestEval = (float) Integer.MIN_VALUE;
        float alpha = (float) Integer.MIN_VALUE;
        float beta = (float) Integer.MAX_VALUE;
        int bestMove = -1;
        ...
        for (int i = 0; i < NxN; i++) {
            ...
            if (board.canPut(s)) {
                flipped = board.setBoard(s);  //BLACKに打たせるときは、-player.getColor()の負号をとり、
                float eval = alphabetaweak(board, 4, -turn, player, alpha, beta);
                board.setAgain(flipped, -turn);  //Board.evaluate で -1 と 1 を入れ替える
                if (eval > bestEval) {    
                    bestEval = eval;
                    bestMove = i;
