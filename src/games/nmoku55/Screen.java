...
class Screen extends Thread implements MouseListener {
    ...
    static boolean repaintFlag = true;
    static Frame frame;
    ...
    public Screen() {    //コンストラクタ       
        ...
        */
        ...
        //フレームfを表示
        frame.setTitle("高岡工芸高校コンピュータ研究部(N目)");
        ...
        winalt.popUp("スタート！[ " + NMOKU + " 目並べ] あなたは黒石");
        //game.start(this);
    }
    ...
    void setFrameTitle(String string){
        if(string==null){
            frame.setTitle("高岡工芸高校コンピュータ研究部");
        }else{
    ...
    void boardState() {
        ...
        for (int y = 0; y < Constants.HEIGHT; y++) {
            for (int x = 0; x < Constants.WIDTH; x++) {
                int cl = game.getBoard().getStone(y * Constants.WIDTH + x).getColor();
